...
@pytest.fixture()
def set_up(
        page):  # def set_up(playwright: Playwright):    NO LO NECESITO XQ PLAYWRIGHT YA VIENE CON PAGE (una fixture propia)
    ...
    yield page
    # page.close()  - Hay que dejar que playwright las cierre sola, sino al correr con --screenshot=only-on-failure no me toma las screenshots
...
def context_creation(playwright):
    ...
    context.storage_state(
        path='state.json')  # Se crea solo un archivo que me guarda todas las cookies y tokens de la pagina, y despues los puedo usar. Me da un mejor test isolation.
    yield context
...
def login_set_up(context_creation, playwright):
    ...
    context = browser.new_context(
        storage_state='state.json')  # Al haber guardado el context en un archivo en la linea 43, ahora lo uso aca y en vez de abrir una nueva tab me abre una nueva pagina por cada test.
    page = context.new_page()
    page.goto("https://practicetestautomation.com")
    page.wait_for_timeout(2000)
    yield page
...
"""ESTE HOOK PERMITE ADJUNTAR UN SCREENSHOT EN CADA TEST QUE FALLE A ALLURE"""
@pytest.hookimpl(hookwrapper=True, tryfirst=True)
def pytest_runtest_makereport(item, call):
    outcome = yield
    result = outcome.get_result()
    if result.failed:  # if call.when == "call" and result.failed: (en casi que quiera que no tome screenshots en el setup o teardown
        page = item.funcargs.get('page', None)
        if page:
            allure.attach(page.screenshot(), name="Captura de pantalla", attachment_type=allure.attachment_type.PNG)
